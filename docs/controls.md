...
| Escape | 再生を停止し、最後のPlay画面で次のViewer命令を待機 |

一時停止中は譜面時計、音声、動画BGA、Play skinの経過時間が同じ位置で停止し、
シークとF5再読込を行っても停止状態を維持します。
シークではロード済みのPCMと変換済み譜面を再利用し、シーク先より前の判定・スコア・
ゲージを初期状態へ戻します。F5だけはエディタで保存した変更を反映するため譜面ファイルを
再importします。待機中にもう一度Escapeを押すと通常のSelect画面へ移動します。
...
Viewer modeではEscapeだけが上表の停止操作になり、E1+E2 hold / E2+E3は即座に
Selectへ戻ります。再生終了後に最後のPlay画面を保持している待機中も、同じ退出操作を
受け付けます。
