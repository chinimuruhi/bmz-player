...
編集・削除、譜面検索からの課題曲追加と並べ替え、段位制約・トロフィーの編集、
beatoraja互換JSONの入出力、保存した内容のテストプレイを行えます。
...
課題曲は同じキーモードで最大5譜面まで追加でき、同じ譜面の重複も可能です。
画面上のパネルでコース名の変更、個別削除、直前の追加取り消し、保存、キャンセルを行えます。
キーボードの `Backspace` / `Delete` は直前の追加を取り消し、`Escape` は作成をキャンセルします。
戻る操作はフォルダ内では1階層戻り、ルートでは作成をキャンセルします。
保存したコースは `COURSE` フォルダへ追加されます。制約・トロフィー・JSON入出力などの詳細編集は
F1メニューの「コース / 段位作成」を使用します。
