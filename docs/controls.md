...
ファイルダイアログを使ったbeatoraja互換JSONの入出力、保存した内容のテストプレイを行えます。
...
画面上のパネルでコース名、制約、トロフィー、IR送信可否の変更、個別削除、直前の追加取り消し、
保存、キャンセルを行えます。トロフィー編集欄は既定で折り畳まれ、新規コースはトロフィーなしです。
キーボードの `Backspace` / `Delete` は直前の追加を取り消し、`Escape` は作成をキャンセルします。
戻る操作はフォルダ内では1階層戻り、ルートでは作成をキャンセルします。
保存したコースは `COURSE` フォルダへ追加されます。課題曲の検索・並べ替えやJSON入出力などの
